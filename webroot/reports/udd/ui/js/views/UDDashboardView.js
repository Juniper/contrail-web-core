...
define([
  ...
  var UDDashboardView = ContrailView.extend({
    ...
    render: function () {
      ...
      this.currentDashboard = urlParams.p.split("_").slice(-1).pop()
        || this.model.dashboardIds()[0]
        || ctwl.TITLE_UDD_DEFAULT_DASHBOARD;

      this.currentTab = urlParams.tab
        || this.model.tabIds(this.currentDashboard)[0]
        || queryEngineUtils.generateQueryUUID().slice(0, 36);
