...
define([
    ...
    function getProtocolName(protocolNumber) {
        var protocol = protocolMap[protocolNumber],
            protocolName;
        if (_.isNull(protocol) || _.isUndefined(protocol)) {
    ...
    function getProtocolCode(protocolName) {
        return _.flow(
            function(pn) {
                return function (protocol) {
                    return protocol.name === pn;
                };
            },
            _.partial(_.find, protocolList),
            function (res) {
                return [res];
            },
            _.partialRight(_.pluck, "value"),
            _.first
        )(protocolName.toUpperCase());
    }

    window.getProtocolName = getProtocolName; // Required by contrail-web-core/webroot/js/analyzer-utils.js
                                              // Remove this global once the above file is converted into a module
    
    return {
        protocolList: protocolList,
        getProtocolName: getProtocolName,
        getProtocolCode: getProtocolCode
    };
});
