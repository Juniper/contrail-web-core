	...
	var ContrailVisView = ContrailView.extend({
    	...
    	render: function (viewConfig) {
            var self = this,
            	...
            	visOptions = viewConfig.visOptions ? viewConfig.visOptions : self.defaultVisOptions;
    	...
    	initNavigations: function(navigations) {
    		...
    		_.each(navigations, function(value, key, obj){
                if(obj && key && obj[key]) {
                    ...
                    $(div).append($("<i class='icon-align-center' />"));
    	...
    	setData: function(data, options) {
    		var self = this;
            /*if(data && data.hasOwnProperty("__kb")) {
                //Data is ContrailModel
                var nodeModels = data.nodesCollection();
                var edgeModels = data.edgesCollection();

                var visNodes = [];
                _.each(nodeModels, function(nodeModel, idx){
                    var nodeConfig = {},
                        node = nodeModel.model().attributes;
                    nodeConfig["id"] = node.element_id();
                    nodeConfig["name"]= node.name();
                    nodeConfig["level"] = node.level();
                    nodeConfig["color"] = node.color();
                    nodeConfig["shape"] = node.shape();
                    nodeConfig["size"] = node.size();
                    nodeConfig["image"] = node.image();                    
                    nodeConfig["font"] = node.font();
                    nodeConfig["labelHighlightBold"] = 
                        node.labelHighlightBold();
                    nodeConfig["label"] = 
                        contrail.truncateText(node.name(), 20);
                    visNodes.push(nodeConfig);
                });
                if(visNodes.length > 0)
                    self.nodesDataSet.add(visNodes);

                var visEdges = [];
                _.each(edgeModels, function(edgeModel, idx){
                    var edgeConfig = {},
                        edge = edgeModel.model().attributes;
                    edgeConfig["id"] = edge.element_id();
                    edgeConfig["from"] = edge.from();
                    edgeConfig["to"] = edge.to();
                    visEdges.push(edgeConfig);
                });
                if(visEdges.length > 0)
                    self.edgesDataSet.add(visEdges);

            } else {
                if(data && data.hasOwnProperty("nodes")) {
                    self.nodesDataSet.add(nodes);            
                }
                if(data && data.hasOwnProperty("edges")) {
