         ...
         showSchemes: function(modalId) {
             ...
             for (var i=0; i<schemeNames.length; i++){
                 ...
                 for ( var n = 0; n < 6; n++ ){
                     ...
                     colors[schemeName][Number(cowc.NUM_OF_COLORS)][n]+
                     " y='"+n*15+"'/>";
                 }
                 svg += "</svg>";
                 $("#palettes").append(palette.append(svg).click( function(){
                     if ( $(this).hasClass("selected") ) return;
                     self.setScheme( $(this).attr("class").split(" ")[1], true);
                 }));
             }

             if (model.selectedScheme instanceof Function && model.selectedScheme().trim() != "" &&
                 $(".palette."+model.selectedScheme())[0] ){
                 self.setScheme( model.selectedScheme(), false);
             }
