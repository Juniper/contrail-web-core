        ...
        configData, callback)
{
    ...
    _.forIn(refsDeltaArrayMap, function(value, key) {
        ...
        _.forEach(addedList, function(ref) {
            prepareRefUpdates(appData, dataObjArrs.add, type, key,
                    ref, 'ADD', uuid);
        });
        _.forEach(deletedList, function(ref) {
            prepareRefUpdates(appData, dataObjArrs.delete, type, key,
                    ref, 'DELETE', uuid);
        });
    });
    if(!dataObjArrs.delete.length && !dataObjArrs.add.length) {
