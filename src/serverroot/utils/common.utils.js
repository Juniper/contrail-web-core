...
{
    // Current session should be destroyed as logging out terminates it.
    req.session.destroy(function (err) {
        if (err) {
            // Add the exception as a warning to server log
            // and let the following logic to proceed still.
...
function getWebServerInfo (req, res, appData)
{
    ...
    var sessionProject = getValueByJsonPath(req,
        'session;last_token_used;project;name', null, false)
    ...
    serverObj['connectedAppsInfo'] = config.connectedAppsInfo;
...
function checkIfReqdPackagesInstalled (packages, req, res) {
    ...
    _.each(packages, function(pkg) {
        if(envVars[pkg] == null || envVars[pkg].toLowerCase() != 'true') {
            retVal = false;
        }
    });
    return retVal;
