...
function getWebServerInfo (req, res, appData)
{
    ...
    authApi.getUserRoleByTenant(userObj, function(err, data) {
        if ((null == err) && (null != data) && (null != data['roles'])) {
            var roles = authApi.getUIRolesByExtRoles(req, data['roles']);
            if (null != roles) {
...
function getUserRoleListPerTenant (req, res, callback)
{
    ...
    if (null != project) {
        ...
        var userObj = {'tokenid': tokenId, 'tenant': project, 'req': req};
        authApi.getUserRoleByTenant(userObj, function(error, tokenData) {
