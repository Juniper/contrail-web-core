...
function getAuthDataByReqUrl (token, authUrl, callback)
{
  ...
  var startIndex = 0;
  getAuthData(dataObjArr, startIndex, makeAuthGetReq, function(err, data) {
    if (null == err) {
        callback(null, data);
    } else {
        callback(err, null);
...
function getTenantList (req, callback)
{
    ...
    var reqUrl = '/tenants';
    getAuthRetryData(token, reqUrl, callback);
}

function getAuthRetryData (token, reqUrl, callback)
{
...
function getAuthData (reqArr, index, authCB, callback)
{
    ...
    authCB(reqArr[index], function(err, data) {
        if ((null == err) && (null != data)) {
            ...
            getAuthData(reqArr, index + 1, authCB, callback);
        }
    });
}
...
function doAuth (username, password, tenantName, callback)
{
    ...
    var startIndex = 0;
    getAuthData(reqArr, startIndex, makeAuthPostReq, function(err, data) {
        if (null == err) {
            callback(data);
        } else {
            callback(null);
