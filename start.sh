...
if [ ! -f "$NODE_LOG_DIR/csui-out.log" ]; then
    touch $NODE_LOG_DIR/csui-out.log
fi

if [ ! -f "$NODE_LOG_DIR/csui-error.log" ]; then
    touch $NODE_LOG_DIR/csui-error.log
fi
...
if [ $NODE_HOT_RELOAD -eq 0 ]; then
    ...
    NCMD="$NCMD -o $NODE_LOG_DIR/csui-out.log"
    NCMD="$NCMD -e $NODE_LOG_DIR/csui-error.log"
else
    NCMD="supervisor -n exit -w ./src,./lib,$NODE_CONFIG_DIR,$NODE_LAUNCH_SCRIPT"
fi
...
if [ $NODE_HOT_RELOAD -eq 0 ]; then
    echo "--------------- NOTE: --------------"
    echo "You can stop the application by running stop.sh (in this folder):"
    echo "------------------------------------"
fi
